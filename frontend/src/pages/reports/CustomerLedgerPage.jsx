...
import { useCustomerLedger, useSundryDropdown } from "../../hooks/useApiQueries";
...
export default function CustomerLedgerPage() {
  ...
  const { data: customers = [] } = useSundryDropdown({ role: "customer" });
  const { data, isLoading, isFetching, error, refetch } = useCustomerLedger({ customerId, search: debouncedSearch, from, to });

  useEffect(() => { if (error) notifyError(error); }, [error]);

  const entries = data?.entries ?? [];
  const totals = data?.totals ?? {};
  const exportLedger = () => downloadCsv("customer-ledger.csv", [
    { header: "Date", value: "date" },
    { header: "Customer", value: "partyName" },
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Invoice debits, payment credits, and receivable balances</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <SummaryCard label="Invoice Debits" value={totals.debit} tone="text-red-600" />
        <SummaryCard label="Payment Credits" value={totals.credit} tone="text-green-600" />
        <SummaryCard label="Net Receivable" value={totals.balance} tone={Number(totals.balance || 0) >= 0 ? "text-red-700" : "text-green-700"} />
      </div>

      <div className="card">
        <div className="card-header flex-wrap gap-3">
          <SearchBar value={search} onChange={setSearch} placeholder="Search customer, invoice, booking..." />
          <select className="input w-56" value={customerId} onChange={(e) => setCustomerId(e.target.value)}>
            <option value="">All customers</option>
            {customers.map((c) => <option key={c._id} value={c._id}>{c.contactPerson}{c.companyName ? ` (${c.companyName})` : ""}</option>)}
          </select>
          ...
          {(from || to || customerId) && <button onClick={() => { setFrom(""); setTo(""); setCustomerId(""); }} className="btn-ghost text-xs"><i className="fa fa-times" /> Clear</button>}
        ...
        {isLoading ? <div className="p-8"><PageLoader /></div> : entries.length === 0 ? (
          <Empty icon="fa-book-open" message="No customer ledger entries found" />
        ) : (
          <div className="table-wrapper">
            <table className="table">
              <thead><tr><th>Date</th><th>Customer</th><th>Reference</th><th>Booking</th><th>Description</th><th>Debit</th><th>Credit</th><th>Balance</th></tr></thead>
              <tbody>
                {entries.map((entry, idx) => (
                  <tr key={`${entry.partyName}-${entry.reference}-${idx}`}>
                    ...
                    <td className="text-red-600 font-semibold">{entry.debit ? money(entry.debit, entry.currency) : "-"}</td>
                    <td className="text-green-600 font-semibold">{entry.credit ? money(entry.credit, entry.currency) : "-"}</td>
